...
func TestUpstreamAuthInterceptor_ForwardsShareOwner(t *testing.T) {
	ctx := auth.AuthSessionTo(context.Background(), interceptorTestSession{
	...
	interceptor := NewUpstreamAuthInterceptor(
		interceptorTestAuthProvider{userID: "visiting-user"},
	...
	if got := req.ServiceParams.Get("X-User-Id"); len(got) != 1 || got[0] != "owner-user" {
		t.Errorf("X-User-Id: want %q, got %q", "owner-user", got)
	}
}
